...
MenuEnterName = Menu.extend({
    ...
    init: function() {
        this.parent();
        if (ig.game.menu && ig.game.menu.scroll) {
            this.scroll = ig.game.menu.scroll;
        }
    ...
    keydown: function(e) {
        if (e.key === 'Backspace') {
            ...
            if (this.name.trim() === '') { this.name = 'Player'; }
            ig.game.playerName = this.name;
            this.close();
            ig.game.setGame();
            ig.game.menu = null;
            e.preventDefault();
        } else if (e.key.length === 1 && this.name.length < 15) {
            this.name += e.key;
            e.preventDefault();
        }
    },
draw: function() {
    ...
    this.font.draw('Enter Your Name', x, y, ig.Font.ALIGN.CENTER);
    this.fontSelected.draw(this.name + '_', x, y + 40, ig.Font.ALIGN.CENTER);
    this.font.draw('Press Enter to Start', x, y + 120, ig.Font.ALIGN.CENTER);
...
ig.module('game.menus.game-over').requires('game.menus.base', 'game.menus.interstitial', 'game.menus.stats').defines(function() {
    ...
    function sendScoreToServer() {
        ...
        if (!playerName || !score || !wave) {
            console.error("Missing data in local storage. Cannot send to Firebase.");
            return;
        }
        ...
        db.collection("scores").add(data)
            .then((docRef) => {
                console.log("Score successfully written to Firebase with ID: ", docRef.id);
            })
            .catch((error) => {
                console.error("Error writing score to Firebase: ", error);
            });
    }
...
ig.module('game.words.en').defines(function() {
    ...
    ig.WORDS.EN ={
  2: [
    'os','ui','ip','db','ai','vr','ux','js','go','ci',
    'cd','ml','it','py','rb','vm','id','up','on','ok',
    'dl','cl','vm','qt','xp','io','rn','ng','tk','ux'
  ],
  3: [
    'api','git','xml','sql','npm','sdk','cli','cpu','ram','ssh',
    'tls','dns','tcp','udp','mac','key','ssl','crt','pkg','dev',
    'sdk','pip','vim','ide','exe','rpm','apt','net','hub','obj'
  ],
  4: [
    'node','java','json','rest','bash','yaml','heap','loop','unix','open',
    'port','ping','fork','exec','sudo','init','grep','user','test','data',
    'load','save','view','args','code','core','repo','tool','edit','lock'
  ],
  5: [
    'react','token','array','debug','stack','model','proxy','fetch','cache','queue',
    'patch','merge','issue','build','serve','shell','class','input','param','value',
    'error','login','email','entry','print','index','scope','layer','query','block'
  ],
  6: [
    'docker','python','binary','github','commit','branch','kernel','socket','stream','object',
    'module','runner','output','search','deploy','server','config','status','logger','import',
    'export','result','memory','editor','script','runner','schema','helper','method','system'
  ],
  7: [
    'network','request','program','process','compile','virtual','package','session','builder','cluster',
    'release','version','license','library','context','monitor','sandbox','logging','upgrade','cleanup',
    'runtime','desktop','channel','storage','command','default','control','project','message','handler'
  ],
  8: [
    'function','database','operator','frontend','workflow','compiler','protocol','variable','security','resource',
    'features','software','commands','language','checking','patching','openrepo','forkable','training','download',
    'migration','terminal','backlogs','generate','solution','priority','platform','abstract','blueprint','tracking'
  ],
  9: [
    'framework','container','algorithm','interface','parameter','structure','iteration','exception','serialize','immutable',
    'debugging','migration','community','scripting','packaging','scheduler','extension','benchmark','repository','contributor',
    'deployment','automation','monitoring','validation','validation','generation','management','navigation','annotation','reflection'
  ],
  10: [
    'javascript','deployment','automation','validation','integration','monitoring','encryption','responsive','evaluation','scheduling',
    'opensource','contribute','maintainer','collabtool','distribution','multithread','compression','versioning','filesystem','component',
    'orchestrat','performance','compatibil','streamline','expression','translation','credential','processing','annotation','allocation'
  ],
  11: [
    'programming','refactoring','configuring','connections','visualizers','compilation','cloudnative','application','credentials','expressions',
    'interpreter','contributors','requirements','integration','translation','development','prototyping','semaphores','superadmins','persistence',
    'optimization','visualgraph','taskrunner','serializer','initializes','environment','collaborate','engineereds','regression','networking'
  ],
  12: [
    'microservice','authentication','virtualizers','synchronizers','implementing','documentations','modularities','refactorings','databasesync','serialization',
    'interactions','contributing','visualmodels','middlewares','dependencies','coordinators','accelerators','transpilerss','environments','compilations',
    'configurator','distributionn','observations','adaptationss','architecture','benchmarkerz','authorization','compatibilty','optimizations','repositories'
  ]
};
});
    ...
    ZType = ig.Game.extend({
        ...
        draw: function() {
            ...
            if (this.menu) {
                ...
                if (this.gameTransitionTimer && this.menu instanceof MenuTitle) {
...
function showNameForm() {
    document.getElementById('name-form-overlay').style.display = 'flex';
    document.getElementById('playerNameInput').focus();
}

// This function hides the form
function hideNameForm() {
    document.getElementById('name-form-overlay').style.display = 'none';
}

// This is the main logic that runs when the page is ready
window.addEventListener('load', function() {
    // Find the "Start Game" button in our new form
    const startGameButton = document.getElementById('startGameBtn');
    const nameInput = document.getElementById('playerNameInput');

    // Define what happens when the button is clicked
    const handleStart = function() {
        const playerName = nameInput.value || 'Player'; // Get the name or use a default
        
        // Save the name to the game object (we still need this temporary storage)
        if (ig.game) {
            ig.game.playerName = playerName;
        }

        hideNameForm(); // Hide the form
